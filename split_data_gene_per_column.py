...
# save training set
outfile_outer_train = 'planc_outer_train' + str(counter) + '.txt'
f = open(outfile_outer_train, 'w')
for line in range(len(outer_y_train)):
    for element in range(len(outer_x_train[line])):
        print(outer_y_train[line], outer_x_train[line][element], file=f)

# save test set
outfile_outer_test = 'planc_outer_test' + str(counter) + '.txt'
f = open(outfile_outer_test, 'w')
for line in range(len(outer_y_test)):
    for element in range(len(outer_x_test[line])):
        print(outer_y_test[line], outer_x_test[line][element], file=f)
